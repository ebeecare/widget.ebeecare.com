        ScheduleOnceEmbedPosition = "1,3,2";
        ScheduleOnceEmbedPhoneTitle = "Call us";
        ScheduleOnceEmbedPhoneText = "+1 650-206-5585_so_phone_sep_US: 800.505.5257";
        ScheduleOnceEmbedEmailTitle = "Email us";
        ScheduleOnceEmbedEmailBText = "Send";
        ScheduleOnceEmbedEmailBTextColor = "rgb(255, 255, 255)";
        ScheduleOnceEmbedEmailBBGColor = "#1f7eb7";
        ScheduleOnceEmbedEmailSendTo = "contact@scheduleonce.com";
        ScheduleOnceEmbedEmailAck = "ScheduleOnce";
        ScheduleOnceEmbedEmailAfter = "Thank you for your message.<br />We will get back to you as soon as we can.";
        ScheduleOnceEmbedBookingTitle = "Schedule online";
        ScheduleOnceEmbedBookingBText = "Book your session now";
        ScheduleOnceEmbedBookingBBGColor = "#1f7eb7";
        ScheduleOnceEmbedBookingBTextColor = "rgb(255, 255, 255)";
        //ScheduleOnceEmbedLink = "https://www.ebeecare.com/booking1";
        ScheduleOnceEmbedLink = "/embed/booking1";
        ScheduleOnceEmbedWidgetTitle = "Ebeecare booking";
        ScheduleOnceEmbedWidgetColor = "rgb(0, 0, 0)";
        ScheduleOnceEmbedWidgetBGColor = "#ff9a00";
        ScheduleOnceEmbedWidgetLeft = "right";
        ScheduleOnceEmbedPageName = "'getso'";
...
var SOE = function() {
    ...
    this.onclickProperty += "'SOWidgetContent','SOWidgetToggle')" + '"';
    this.phoneTitle;
    this.phoneText;
    this.phoneTextSep = "_so_phone_sep_";
    this.bookingTitle;
    this.bookingBText;
    ...
    this.URL = "https://secure.scheduleonce.com";
    this.isWidget = false;
    this.contactUsDiv = "";
    this.prevTop = 0;
    this.prevLeft = 
...
SOE.prototype.getWidgetCode = function() {
    ...
    <div id="SOWidgetTitle" style="' + widgetHeadStyle + "background-color:" + (this.widgetBGColor == "" ? "#333" : this.widgetBGColor) + ';">\
    <div align="left" style="float:left;width:80%"><img src="ebk2.png" ></div>' 
    + '<div id="SOWidgetToggle" style="' + openCloseHolderClass + '" >+</div></div></a>';
    this.code += '<div id="SOWidgetContent" style="height:100%;display:none;margin-top:0.7em;box-sizing:content-box;">';
...
SOE.prototype.setWidgetHTML = function() {
    this.getBookingCode();
    this.getWidgetCode();
    var ele = document.getElementById("SOWidget");
    ...
    if (this.isMobileView) {
        ...
        document.getElementById("SOWidgetTitle").style.display = "none";
        document.getElementById("mSOWidgetTitle").style.display = "block";
        document.getElementById("SODesktopPower").style.display = "none";
        document.getElementById("SOMobilePower").style.display = 
        "block";
        document.getElementById("SOWidget").style.display = "none";
        document.getElementById("SOWidget").style.width = "80%";
        document.getElementById("SOWidget").style.left = "10%";
        setTimeout(function() {
...
SOE.prototype.toggleDiv = function(id, obj, hideOnly) {
    ...
    if (elem) {
        var widgetDiv = id == "SOWidgetContent" ? true : false;
        if ((elem.style.display == "none" || (elem.style.display == "" || elem.style.display == "undefined")) && hideOnly == null ) {
            elem.style.display = "block";
            if (widgetDiv) {
                if (this.isMobileView) {
                    document.getElementById("mSOWidgetTitle").style.display = "none";
                    document.getElementById("SOWidgetTitle").style.display = "block";
                    document.getElementById("SOWidget").style.display = "block";
                    if (this.isLive) {
                        document.getElementById("SoWidgetBackground").style.display = 
                        "block";
                        window.scrollTo(0, 0)
                    }
                }
            ...
            if (widgetDiv) {
                if (this.isMobileView) {
                    ...
                    document.getElementById("SOWidget").style.display = "none";
                    if (this.isLive)
                        document.getElementById("SoWidgetBackground").style.display = "none";
                    else
                        document.getElementById("mSOWidgetTitle").style.bottom = "0%"
                }
                if (!this.isLightBox && 
                !this.isMobileView)
                    this.TransitionCode(document.getElementById("SOWidget"));
                if (this.prevHtml != "")
                    document.getElementById("SOWidgetContent").innerHTML = this.prevHtml
            }
            if (obj != null )
                document.getElementById(obj).innerHTML = "+"
        }
...
SOE.prototype.toggleLightBox = function(page) {
    ...
    if (this.isMobileView || ieversion < 9) {
        ...
        if (ele)
            ...
            var allIfrm = document.querySelectorAll("[name*='ScheduleOnceIframe_" + page + "']");
        ...
        -1) {
            ...
            this.toggleDiv("SOWidgetContent", "SOWidgetToggle", true);
            if (ieversion >= 9) {
            ...
            if (ieversion >= 9) {
                ...
                "px;top:" + blanketTop + "px;width:" + _width + ";z-index:100000001;position:fixed;overflow-y:auto";
            ...
            var topVal = "top:15px;";
            if (ieversion >= 9) {
                divStyle = divStyle.replace("width:" + _width + ";z-index:100000001;position:fixed;overflow-y:auto", "width:775px;" + topVal).replace("height:100%", height) + "position:absolute;";
                ...
                childEle.style.zIndex = "100000001";
...
SOE.prototype.getIframeHTMLCode = function(_pageLink, isParameterized) {
    ...
    if (this.isLive)
        ...
        iframesrc = src;
    var _code = pre + '<iframe src="' + iframesrc + '" id="SOI_' + this.pageName.replace(/'/g, "") + '" name="ScheduleOnceIframe" data-mysrc="' + mysrc + '" frameborder="0" hspace="0" marginheight="0" marginwidth="0" height="' + h + '" width="' + w + '" vspace="0" style="' + style + '"></iframe>' + post;
    return _code
}
;

SOE.prototype.encodeToHTML = function(str, removeLines) {
    if (str) {
        if (removeLines)
...
;
SOE.prototype.fixLines = function(str) {
    return str.replace(/line_break_placeHolder/g, "<br />")
}
;
...
;
SOE.prototype.createRequestObject = function() {
...
;
SOE.prototype.onBlurText = function(obj) {
    if (obj.id == "SONote")
        if (this.ValidateQuery(obj.value)) {
            document.getElementById("SONoteErr").style.display = "none";
            obj.style.color = "#161616"
        } else {
            obj.style.color = "#666666";
            document.getElementById("SONoteErr").style.display = "block"
        }
}
;
SOE.prototype.ValidateQuery = function(query) {
    if (query.length < 1 || query == "Your note") {
        document.getElementById("SONoteErr").innerHTML = "Note is a mandatory field";
        return false
    } else if (query.length > 200) {
        document.getElementById("SONoteErr").innerHTML = "Maximum 200 characters allowed";
        return false
    } else
        return true
}
...
SOE.prototype.processOrientationChange = function(updateWidth) {
    ...
    if (!soe.isMobileView) {
        ...
        var ele = 
        document.getElementById("SOWidget");
        if (ele) {
            ...
            if (updateWidth) {
                ...
                document.getElementById("mSOWidgetTitle").style.fontSize = 
                mCalcFont + "px"
            }
        }
    }
...
SOE.prototype.zoomChange = function() {
    ...
    if (ele) {
        ...
        if (soe.runCode && soe.oldScreenCssPixelRatio != screenCssPixelRatio) {
            ...
            ele.style.fontSize = mCalcFont + "px";
            document.getElementById("mSOWidgetTitle").style.fontSize = mCalcFont + "px"
        }
    }
}
;
...
SOE.prototype.setIframeOnloadEvent = function() {
    var allIfrm = document.querySelectorAll("[name*='ScheduleOnceIframe']");
    if (allIfrm.length > 0)
    ...
    if (allIfrm.length > 0)
        for (var i = 0; i < allIfrm.length; i++)
            if (allIfrm[i].parentNode.querySelectorAll("*[name='SOLoadingMessage']").length == 0) {
                ...
                (mCss ? 250 : 350) + 'px;  height: 50%;  background-color: #fff;  margin: 0 auto;"><h2 id="LoadingText" style="margin-bottom: 0px; text-align: center;font-size: ' + (mCss ? 16 : 18) + 'px; color: #333 !important;font-family: Tahoma, Verdana, Segoe, sans-serif;letter-spacing: 0rem;">Loading...</h2><div style="height: 20px;"></div><div style="width:100%;"><img id="loadingImage" alt="Loading..." style=" width: 100%; max-width:351px; height: 10px;" src="' + this.URL + '/Images/load_ani.gif" border="0" width="351" height="10"><div id="circle"><div id="circle_1" class="circle"></div><div id="circle_2" class="circle"></div><div id="circle_3" class="circle"></div><div id="circle_4" class="circle"></div><div id="circle_5" class="circle"></div><div id="circle_6" class="circle"></div><div id="circle_7" class="circle"></div><div id="circle_8" class="circle"></div><div id="circle_9" class="circle"></div><div id="circle_10" style="margin-right: 0px;" class="circle"></div><div class="clearfix"></div></div></div><div style="height: 20px;"></div><div id="timetxt" style="display: none"><span style="color: red;">This may take up to 30 seconds.<br>Do not close or refresh this page.</span></div></div></div></div></div></div></div>';
...
SOE.prototype.renameDulplicateIframes = function() {
    ...
    if (allIfrm.length > 1)
        for (var i = 0; i < allIfrm.length; i++) {
            ...
            else if (iframes.length == 1 && allIfrm[i].name == "ScheduleOnceIframe")
        ...
        for (var i = 0; i < allDuplicateIframes.length; i++) {
            ...
            if (sameDuplicates.length > 1)
                for (var j = 0; j < sameDuplicates.length; j++) {
                    ...
                    sameDuplicates[j].name = "ScheduleOnceIframe" + "_" + sameDuplicates[j].id.substring("SOI_".length);
...
SOE.prototype.AddEventListners = function(link, embPageName, height, width, isParameterizedUrl) {
    ...
    var f = document.querySelectorAll("[name*='ScheduleOnceIframe']");
    while (f == null  || f == "undefined")
        setTimeout(function() {
            f = document.querySelectorAll("[name*='ScheduleOnceIframe']")
        }, 33);
...
SOE.prototype.getIframeHTML = function(soqueryparam, link, pageName, h, w) {
    ...
    if (this.isMobileView)
        ScheduleOnceEmbedLink = ScheduleOnceEmbedLink.replace("&dt=", "&dt=0") + "&m=true";
    var code = '<iframe src="' + ScheduleOnceEmbedLink + soqueryparam + '" id ="' + id + '" name="ScheduleOnceIframe"  scrolling="no" frameborder="0" hspace="0" marginheight="0" marginwidth="0" height="' + 
...
SOE.prototype.SetBasicParam = function(ScheduleOnceEmbedLink, ScheduleOnceEmbedPageName, ScheduleOnceEmbedBookingBText, ScheduleOnceEmbedBookingBBGColor, ScheduleOnceEmbedBookingBTextColor) {
    ...
    if (typeof ScheduleOnceEmbedLink != "undefined" && (ScheduleOnceEmbedLink && typeof ScheduleOnceEmbedPageName != "undefined")) {
        soe.link = ScheduleOnceEmbedLink;
        soe.pageName = "'" + 
        soe.decodeToHTML(ScheduleOnceEmbedPageName) + "'";
        soe.isWidget = true;
        try {
            soe.screenwidth_swh = window.innerWidth;
            soe.isNewPage = NewPage
        } catch (err) {}
        soe.bookingBText = ScheduleOnceEmbedBookingBText;
        soe.bookingBBGColor = ScheduleOnceEmbedBookingBBGColor;
        soe.bookingBTextColor = ScheduleOnceEmbedBookingBTextColor
    }
}
;
SOE.prototype.InitButton = function(ScheduleOnceEmbedLink, ScheduleOnceEmbedPageName, ScheduleOnceEmbedBookingBText, ScheduleOnceEmbedBookingBBGColor, ScheduleOnceEmbedBookingBTextColor, isParameterized) {
    try {
        soe.SetBasicParam(ScheduleOnceEmbedLink, ScheduleOnceEmbedPageName, ScheduleOnceEmbedBookingBText, ScheduleOnceEmbedBookingBBGColor, ScheduleOnceEmbedBookingBTextColor);
        soe.setButtonHTML(isParameterized)
    } catch (err) {
...
;
SOE.prototype.InitWidget = function(ScheduleOnceEmbedLink, ScheduleOnceEmbedPageName, ScheduleOnceEmbedBookingBText, ScheduleOnceEmbedBookingBBGColor, ScheduleOnceEmbedBookingBTextColor, ScheduleOnceEmbedPosition, ScheduleOnceEmbedPhoneTitle, ScheduleOnceEmbedPhoneText, ScheduleOnceEmbedEmailTitle, ScheduleOnceEmbedEmailBText, ScheduleOnceEmbedEmailBTextColor, ScheduleOnceEmbedEmailBBGColor, ScheduleOnceEmbedEmailSendTo, ScheduleOnceEmbedEmailAck, ScheduleOnceEmbedEmailAfter, ScheduleOnceEmbedEmailCustTemp, ScheduleOnceEmbedEmailUserTemp, 
ScheduleOnceEmbedBookingTitle, ScheduleOnceEmbedWidgetTitle, ScheduleOnceEmbedWidgetColor, ScheduleOnceEmbedWidgetBGColor, ScheduleOnceEmbedWidgetLeft, ScheduleOnceEmbedAid) {
    try {
        soe.SetBasicParam(ScheduleOnceEmbedLink, ScheduleOnceEmbedPageName, ScheduleOnceEmbedBookingBText, ScheduleOnceEmbedBookingBBGColor, ScheduleOnceEmbedBookingBTextColor);
        soe.bookingTitle = ScheduleOnceEmbedBookingTitle;
        soe.bookingPos = ScheduleOnceEmbedPosition.split(",")[0];
        soe.phonePos = 
        ScheduleOnceEmbedPosition.split(",")[2];
        ...
        soe.widgetTitle = ScheduleOnceEmbedWidgetTitle;
        soe.widgetColor = ScheduleOnceEmbedWidgetColor;
        soe.widgetBGColor = ScheduleOnceEmbedWidgetBGColor;
        soe.widgetLeft = ScheduleOnceEmbedWidgetLeft;
        soe.eaid = typeof ScheduleOnceEmbedAid == "undefined" ? "" : ScheduleOnceEmbedAid;
        soe.phoneTitle = ScheduleOnceEmbedPhoneTitle;
        soe.phoneText = ScheduleOnceEmbedPhoneText;
        soe.setWidgetHTML()
    } catch (err) {
        if (typeof console == "object")
            console.log(err)
...
try {
    ...
    if (typeof ScheduleOnceEmbedLink != "undefined" && (ScheduleOnceEmbedLink && typeof ScheduleOnceEmbedPageName != "undefined")) {
        soe.link = ScheduleOnceEmbedLink;
        soe.pageName = soe.decodeToHTML(ScheduleOnceEmbedPageName);
        soe.isWidget = true;
        try {
            soe.screenwidth_swh = window.innerWidth;
            soe.isNewPage = NewPage
        ...
        soe.bookingBBGColor = ScheduleOnceEmbedBookingBBGColor;
        soe.bookingBTextColor = ScheduleOnceEmbedBookingBTextColor;
        if (typeof ScheduleOnceEmbedPosition != "undefined") {
            soe.bookingTitle = ScheduleOnceEmbedBookingTitle;
            soe.bookingPos = ScheduleOnceEmbedPosition.split(",")[0];
            soe.phonePos = ScheduleOnceEmbedPosition.split(",")[2];
            soe.displayCount = soe.bookingPos < 1 ? soe.displayCount - 1 : soe.displayCount;
            soe.displayCount = soe.phonePos < 1 ? soe.displayCount - 1 : soe.displayCount;
            soe.widgetTitle = ScheduleOnceEmbedWidgetTitle;
            soe.widgetColor = ScheduleOnceEmbedWidgetColor;
...
function receiveMessage(event) {
    if (event.origin.indexOf("scheduleonce") < 0 && (event.origin.indexOf("staticso") < 0 && (event.origin.indexOf("booknow.so") < 0 && (event.origin.indexOf("meetme.so") < 0 && event.origin.indexOf("bookme.so") < 0))))
        return false;
    var tmp = event.data.toString();
    if (tmp.indexOf("ScheduleOnceifrId") > -1) {
        var ScheduleOnceifrId = null ;
        var height = 725;
        var recM = false;
        ...
        if (ScheduleOnceifrId != null ) {
            var id = "SOI_" + ScheduleOnceifrId;
            if (mobileView == true)
                height = height < 280 ? 280 : height;
            var innerLightBoxHeight = height + 40;
            var SOI_ele = document.querySelectorAll("[name*='ScheduleOnceIframe']");
...
MasterPageSOLightBox = function(_pageLink, pageName) {
    var h = "100%";
    var w = "90%";
    var ver = "";
    var style = "border:solid 3px #FFF;background-color:#FFF;min-width:769px;";
    var pre = '<div id="SOIC_' + pageName.replace(/'/g, "") + '" style="padding: 0px; margin: 0px;width: 24px;height: 24px; background-color: #000; border: solid 2px #FFF;color:#FFF;font-size: 18px;font-weight: bold;font-family: tahoma; -webkit-border-radius: 14px; border-radius: 14px; position: absolute; top: 0px; text-align:center; line-height:23px;cursor:pointer;box-sizing:content-box; right:-17px" onclick="soe.toggleLightBox(\'' + 
    pageName + "')\">X</div>";
    pre = '<div align="center" style="height:100%; padding-top: 15px; padding-right: 15px; box-sizing: border-box; overflow: hidden;">' + pre;
    var post = "";
    h = "535px";
    w = "100%";
    ver = "";
    style = "width:" + w + ";height=" + h + ";border-radius: 7px;-webkit-border-radius:7px;" + style;
    post = "</div>";
    //var _code = pre + '<iframe src="' + _pageLink + "&dt=" + ver + '&em=1" id="SOI_' + pageName.replace(/'/g, "") + '" name="ScheduleOnceIframe"  scrolling="no" frameborder="0" hspace="0" marginheight="0" marginwidth="0" height="' + 
    var _code = pre + '<iframe src="' + _pageLink + ' id="SOI_' + pageName.replace(/'/g, "") + '" name="ScheduleOnceIframe"  scrolling="no" frameborder="0" hspace="0" marginheight="0" marginwidth="0" height="' + 
    h + '" width="' + w + '" vspace="0" style="' + style + '"></iframe>' + post;
    return '<div id="SOLightBox' + pageName.replace(/'/g, "") + '" name="SOLightBox" style="top:-10000px; position:fixed; padding:0px; margin:0px;"><div id="SOInnerLightBox">' + _code + "</div></div>"
}
;
function TrimWhiteSpaces(element) {
    if (element) {
        var temp = element.value.trim();
        element.value = "temp@value.com";
        element.value = temp
    }
}
;
